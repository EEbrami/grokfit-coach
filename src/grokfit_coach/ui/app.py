...
def generate_plan(profile_state: dict | None):
    """Generate an improved plan using the agent (triggered from Plans tab)."""
    if profile_state:
        try:
            profile = _form_values_to_profile(profile_state)
        except Exception:
    ...
    try:
        # Use a natural language trigger so the graph's conditional routes to maybe_plan
        result = invoke_coach(profile, "Please create a weekly workout plan for me based on my profile.")
        plan = result.get("plan")
        if plan:
            save_plan(plan)
            return _format_plan_for_chat(plan), f"Plan generated and saved for {profile.name}."
        else:
            # Fallback message if agent didn't produce one
            return "No structured plan was produced. Try a more explicit request in the Chat tab or adjust your profile.", "Plan generation did not yield a structured result."
    except Exception as e:
        return f"Error generating plan: {e}", "See error above."
...
def launch():
    ...
    with gr.Blocks(title="GrokFit Coach (Local)", theme=gr.themes.Soft()) as demo:
        ...
        with gr.Tab("Plans"):
            gr.Markdown("### Weekly Workout Plans\nGenerate or view your latest plan. Plans are created using the improved Phase 2 logic (profile-aware RAG + constraints + validation).")
            plan_display = gr.Markdown("No plan generated yet. Use the button below or ask in the Chat tab.")
            gen_btn = gr.Button("Generate / Regenerate Weekly Plan", variant="primary")
            ...
            # Also allow loading last saved plan on tab load
            def _load_last_plan():
                plan = load_plan()
                if plan:
                    return _format_plan_for_chat(plan), "Loaded last saved plan."
